...
const BreakdownControls = ({
  ...
  return (
    <div className="space-y-2">
      ...
      {matchSourceSavings && wealthGapUSD > 0 && (
        <div className={`px-4 py-2 rounded-lg text-xs ${theme.name === 'Sunrise' ? 'bg-rose-50 text-rose-700 border border-rose-200' : 'bg-rose-500/10 text-rose-300 border border-rose-500/30'}`}>
          <strong>Wealth Gap:</strong> Destination's pre-tax retirement vehicles can't fully match source's savings. Shortfall: {fmtAmount(wealthGapUSD, displayCurrency)} per year.
        </div>
      )}
    </div>
  );
