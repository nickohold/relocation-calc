...
export const AU_MLS_SINGLE = [
  { max: 101000, rate: 0.00 },
  { max: 118000, rate: 0.01 },
  { max: 158000, rate: 0.0125 },
  { max: Infinity, rate: 0.015 },
];
export const AU_SUPER_RATE = 0.12;
export const AU_SUPER_CONCESSIONAL_CAP = 30000;

const calcMLS = (income) => {
  // MLS is a flat % of full income at the right tier (not progressive).
...
export const compute = ({
  ...
  void eeOtherPct;
  // Default: super is employer-paid (off-payroll). EE pension treated as salary sacrifice if >0.
  const eePension = Math.min(grossLocal * (eePensionPct / 100), AU_SUPER_CONCESSIONAL_CAP);
  ...
  return {
    ...
    netLocal, erContributions, totalSavingsLocal: totalSavings,
    ...
    breakdown: [
      ...
      { label: 'Super (ER)', amount: erContributions, kind: 'pension' },
      { label: 'Salary Sacrifice (EE)', amount: eePension, kind: 'pension' },
    ],
  };
};
