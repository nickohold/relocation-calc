...
//   - https://www.comunidad.madrid/ (Madrid regional brackets)
//   - https://atc.gencat.cat/ (Catalonia regional brackets)
...
export const ES_STATE_BRACKETS = [
  { max: 12450, rate: 0.095 },
  { max: 20200, rate: 0.12 },
  { max: 35200, rate: 0.15 },
  { max: 60000, rate: 0.185 },
  { max: 300000, rate: 0.225 },
  { max: Infinity, rate: 0.245 },
];

export const ES_MAD_BRACKETS = [
  { max: 13362, rate: 0.085 },
  { max: 18004, rate: 0.107 },
  { max: 35425, rate: 0.128 },
  { max: 57320, rate: 0.174 },
...
export const compute = ({
  ...
  void eeOtherPct;
  const eePension = Math.min(grossLocal * (eePensionPct / 100), ES_PENSION_INDIV_CAP);
  const ss = Math.min(grossLocal, ES_SS_BASE_CAP_ANNUAL) * ES_SS_RATE;

  const taxable = Math.max(0, grossLocal - eePension - ss - ES_PERSONAL_ALLOWANCE);
